...
def visualize_detailed_results(model, image, target, device, checkpoint_name, extra_wurscht=False):
    ...
    if extra_wurscht:
        upsampled_logits = nn.functional.interpolate(prediction.logits, size=target.shape[-2:], mode="bilinear", align_corners=False)
    ...
    target_mask_rgb = mask_to_rgba(target, color_map=cmap, bg_channel=-1, alpha=0.6)
    pred_mask_rgb = mask_to_rgba(classes_per_channel, color_map=cmap, bg_channel=-1, alpha=0.6)
...
def visualize_detailed_results_overlay(image_base_list, target_list, pred_dict):
    plt.style.use(["science", "nature"])
    ...
    model_count = len(pred_dict)
    fig, axs = plt.subplots(nrows=model_count + 2, ncols=5, figsize=(9, 10))
    ...
    for i in range(5):
        image = image_base_list[i].cpu()
        image_np = image.permute(1, 2, 0).squeeze().numpy()
        target_mask_rgba = mask_to_rgba(target_list[i], color_map=cmap, bg_channel=3, alpha=0.6)

        axs[1, i].imshow(image_np)
        axs[1, i].imshow(target_mask_rgba.permute(1, 2, 0))
    ...
    axs[0, 0].set_ylabel("Image", fontsize=12)
    axs[1, 0].set_ylabel("Ground Truth", fontsize=12)


    for j, (model_name, predictions) in enumerate(pred_dict.items()):
        j = j +2
        for i in range(5):
            image = image_base_list[i].cpu()
            classes_per_channel = predictions[i].cpu()
            image_np = image.permute(1, 2, 0).squeeze().numpy()

            pred_mask_rgba = mask_to_rgba(
                classes_per_channel, color_map=cmap, bg_channel=3, alpha=0.6
            )
            axs[j, i].imshow(image_np)
            axs[j, i].imshow(pred_mask_rgba.permute(1, 2, 0))
            if (i == 0):
                axs[j, i].set_ylabel(model_name, fontsize=12)
            ...
            fig.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, 0.01), ncol=3, fontsize=12)
